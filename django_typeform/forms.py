...
class TypeformMixin(object):
    ...
    def typeform_to_query_dict(self, typeform_uid):
        """Get repsonse from typeform respopnses api as a query dict suitable for POST."""
    ...
    def render(self, context):
        """
        Renders the typeform. Allows to use "{% include typform %}" in templates for
        typeform rendering
        """
        request = context['request']
        context.update({
                'typeform_uid':     self.get_uid(request),
